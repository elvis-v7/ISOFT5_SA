...
class AppStateSingleton:
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            print("Creando nueva instancia del estado de la aplicación...")
    ...
    def agregar_usuario(self, correo, nombre):
        """Agregar usuario al registro"""
        self.usuarios_registrados[correo] = nombre
    
    def usuario_existe(self, correo):
        """Verificar si usuario existe"""
        return correo in self.usuarios_registrados
    
    def obtener_usuario(self, correo):
        """Obtener nombre de usuario"""
        return self.usuarios_registrados.get(correo)
...
# Crear instancia global del singleton
app_state = AppStateSingleton()

# Validaciones (funciones independientes)
def validar_nombre(nombre):
    return nombre.isalpha()

def validar_correo(correo):
    return re.match("^[A-Za-z0-9]+@[A-Za-z0-9]+\.[A-Za-z]+$", correo) is not None

# Registro de usuario usando el singleton
def registrar_usuario():
    ...
    if app_state.usuario_existe(correo):
        print("⚠️ Ya existe un usuario con este correo.")
        return None, None
    else:
        app_state.agregar_usuario(correo, nombre)
        print(f"✅ Registro exitoso. ¡Bienvenido, {nombre}!")
        return correo, nombre

# Mostrar menú de productos usando el singleton
def mostrar_precios():
    print("\n📋 Lista de Precios")
...
# Inicio del sistema (FUNCIÓN PRINCIPAL)
def inicio():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            if app_state.usuario_existe(correo):
                nombre = app_state.obtener_usuario(correo)
            ...
            print(f"\n📊 Estadísticas:")
            print(f"Usuarios registrados: {len(app_state.usuarios_registrados)}")
            print(f"Pedidos realizados: {len(app_state.obtener_pedidos())}")
        elif opcion == "6":
